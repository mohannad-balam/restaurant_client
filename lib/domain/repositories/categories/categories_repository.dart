...
class CategoriesRepository extends ICategoriesRepository{
  ...
  Future<List<CategoryEntity>> getAll() async{
    try{
      List<CategoryModel> categoriesModel = await locator<ICategoriesService>().getAll();
      return categoriesModel.map((e) => CategoryMapper.toCategoryEntity(e)).toList();
    }catch(e){
      rethrow;
    }
  }
  ...
  Future<List<MenuEntity>> getCategoryMenu(String id) async{
    try{
      List<MenuModel> categoryMenu = await locator<ICategoriesService>().getCategoryMenu(id);
